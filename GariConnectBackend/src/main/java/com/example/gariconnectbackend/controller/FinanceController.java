...
//        }
//
//        // CORRECTION : Prise en compte de AGENCY_ADMIN en plus de AGENCY_MANAGER
//        User agence = (userConnecte.getRole() == Role.AGENCY_MANAGER || userConnecte.getRole() == Role.AGENCY_ADMIN)
//                ? userConnecte
...
//
//    /**
//     * 🔥 LOGIQUE CORRIGÉE : Filtrage du Livre de Caisse selon le rôle
//     */
//    @GetMapping("/livre-de-caisse")
//    public ResponseEntity<?> getReport() {
//        try {
...
//                    .orElseThrow(() -> new RuntimeException("Utilisateur non trouvé"));
//
//            List<LivreDeCaisseRow> reportData;
//
//            // 1. Si SUPER_ADMIN, on affiche le livre de caisse global
//            if (userConnecte.getRole() == Role.SUPER_ADMIN) {
//                reportData = financeService.generateLivreDeCaisseGlobal();
//            }
//            // 2. Sinon, on isole strictement les données de l'agence
//            else {
//                User agence = (userConnecte.getRole() == Role.AGENCY_MANAGER || userConnecte.getRole() == Role.AGENCY_ADMIN)
//                        ? userConnecte
//                        : userConnecte.getAgenceEmployeur();
...
//                            .body(Map.of("message", "Accès refusé. Vous n'êtes rattaché à aucune agence."));
//                }
//
//                // Appel de la méthode ciblée dans le Service
//                reportData = financeService.generateLivreDeCaisse(agence);
//            }
//
...
//            e.printStackTrace();
//            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
//                    .body(Map.of("message", "Erreur lors de la génération du livre de caisse : " + e.getMessage()));
//        }
//    }
//
//    @PostMapping("/transactions")
//    @PreAuthorize("hasAnyRole('AGENCE','SUPER_ADMIN', 'AGENCY_MANAGER', 'AGENCY_ADMIN')")
//    public ResponseEntity<?> addTransaction(@RequestBody FinanceTransaction transaction) {
//        try {
//            String email = SecurityContextHolder.getContext().getAuthentication().getName();
//            User userConnecte = userRepository.findByEmail(email)
...
//                            .orElseThrow(() -> new RuntimeException("L'agence spécifiée par le Super Admin n'existe pas"));
//                } else {
//                    return ResponseEntity.badRequest().body(Map.of("message", "En tant que SUPER_ADMIN, vous devez spécifier une agence destinataire."));
//                }
//            }
//
//            if (agenceCible == null) {
//                return ResponseEntity.badRequest().body(Map.of("message", "Impossible de lier cette transaction à une agence valide."));
//            }
//
//            transaction.setAgence(agenceCible);
//
//            if (transaction.getTypeTransaction() == null) {
//                return ResponseEntity.badRequest().body(Map.of("message", "Le type de transaction (ENTREE ou SORTIE) est obligatoire."));
//            }
//
//            transaction.setTypeTransaction(transaction.getTypeTransaction().toUpperCase());
//
...
public class FinanceController {
    ...
    /**
     * 🛡️ MÉTHODE HELPER UNIQUE : Résout de manière cohérente l'agence propriétaire
     * Évite les doublons de logique et assure que l'Admin et le Manager partagent le même historique.
     */
    private User resolveAgence(User user) {
        if (user == null) return null;

        // 1. S'il est rattaché à un employeur/agence mère, c'est cette entité qui détient les finances
        if (user.getAgenceEmployeur() != null) {
            return user.getAgenceEmployeur();
        }

        // 2. S'il n'a pas d'employeur mais a un rôle d'encadrement d'agence, il est le compte racine
        if (user.getRole() == Role.AGENCY_MANAGER || user.getRole() == Role.AGENCY_ADMIN) {
            return user;
        }

        return null; // Cas du SUPER_ADMIN (qui n'a pas d'agence propre)
    }

    /**
     * Récupérer les transactions simples
     */
    @GetMapping("/transactions")
    public ResponseEntity<?> getTransactions() {
        String email = SecurityContextHolder.getContext().getAuthentication().getName();
        ...
        User agence = resolveAgence(userConnecte);
        if (agence == null) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(Map.of("message", "Accès refusé. Aucune agence associée."));
        }
    ...
    public ResponseEntity<?> updateTransaction(@PathVariable Long id, @RequestBody FinanceTransaction details) {
        ...
        if (userConnecte.getRole() != Role.SUPER_ADMIN) {
            User agence = resolveAgence(userConnecte);
            if (agence == null || transaction.getAgence() == null || !transaction.getAgence().getId().equals(agence.getId())) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Map.of("message", "Accès refusé."));
            }
        }
    ...
    public ResponseEntity<?> getReport() {
        try {
            ...
            if (userConnecte.getRole() == Role.SUPER_ADMIN) {
                ...
                if (agence == null) {
                    return ResponseEntity.status(HttpStatus.FORBIDDEN)
                            .body(Map.of("message", "Accès refusé. Vous n'êtes rattaché à aucune agence active."));
                }
    ...
    @PostMapping("/transactions")
    @PreAuthorize("hasAnyRole('SUPER_ADMIN', 'AGENCY_MANAGER', 'AGENCY_ADMIN')")
    public ResponseEntity<?> addTransaction(@RequestBody FinanceTransaction transaction) {
        try {
            String email = SecurityContextHolder.getContext().getAuthentication().getName();
            User userConnecte = userRepository.findByEmail(email)
            ...
            if (userConnecte.getRole() == Role.SUPER_ADMIN) {
                if (transaction.getAgence() != null && transaction.getAgence().getId() != null) {
                    agenceCible = userRepository.findById(transaction.getAgence().getId())
                            .orElseThrow(() -> new RuntimeException("L'agence spécifiée n'existe pas."));
                } else {
                    return ResponseEntity.badRequest().body(Map.of("message", "En tant que SUPER_ADMIN, vous devez spécifier une agence destinataire."));
                }
            } else {
                agenceCible = resolveAgence(userConnecte);
            }
