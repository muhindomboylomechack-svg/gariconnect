...
//import com.example.gariconnectbackend.dto.ReservationRequest;
...
//
//    @Transactional
//    public Reservation creerEtValiderReservation(ReservationRequest request, User user) {
//        // 1. Sauvegarder la réservation
//        Reservation reservation = new Reservation();
//        // ... initialisation des champs de la réservation ...
//        reservation = reservationRepository.save(reservation);
//
//        // 2. Générer autant de tickets que de places réservées
//        for (int i = 0; i < request.getNombrePlaces(); i++) {
//            Ticket ticket = new Ticket();
//            ticket.setReservation(reservation);
//            ticket.setUser(user);
//
//            // Code unique pour le ticket
//            String codeUnique = "TCK-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase();
//            ticket.setCodeTicket(codeUnique);
//
//            // Construction du contenu du QR Code pour le scanner
//            String payloadQrCode = String.format(
//                    "{\"ticketCode\":\"%s\",\"reservationId\":%d,\"userId\":%d}",
//                    codeUnique, reservation.getId(), user.getId()
//            );
...
//        });
//
//        User agence = reservation.getTrajet() != null ? reservation.getTrajet().getAgence() : null;
//        boolean isAbonnementDefinitif = agence != null && "DEFINITIF".equalsIgnoreCase(agence.getTypeAbonnement());
//
//        double nouvelleCommission = 0.0;
//        Double taux = 0.0;
...
//        });
//
//        // 🎟️ CRÉATION ET ENREGISTREMENT DU TICKET DANS LA TABLE 'tickets'
//        int nbPlaces = (reservation.getNombrePlaces() != null && reservation.getNombrePlaces() > 0)
//                ? reservation.getNombrePlaces() : 1;
//
//        for (int i = 0; i < nbPlaces; i++) {
//            Ticket ticket = new Ticket();
//            ticket.setReservation(reservationMiseAJour);
//            ticket.setStatut("VALIDE");
//
//            // Réutilisation du code de ticket de la réservation ou génération d'un code unique
//            String codeTicket = (nbPlaces == 1 && reservation.getCodeTicket() != null && !reservation.getCodeTicket().isEmpty())
//                    ? reservation.getCodeTicket()
//                    : "TCK-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase();
//
//            ticket.setCodeTicket(codeTicket);
//
//            // Sauvegarde directe dans la table 'tickets'
//            ticketRepository.save(ticket);
//        }
//
//        String detailsTrajet = (reservation.getTrajet() != null) ?
//                "le trajet " + reservation.getTrajet().getDepart() + " - " + reservation.getTrajet().getDestination() : "votre voyage";
//
//        notifierLeClient(reservation.getClient(), "🎉 Paiement confirmé ! Votre billet pour " + detailsTrajet + " est maintenant disponible. Bon voyage !");
//
//        return reservationMiseAJour;
//    }
//}
...
public class ReservationService {
    ...
    public void annulerReservation(Long id) {
        ...
        String statutActuel = r.getStatut() != null ? r.getStatut().toUpperCase() : "";

        // 2. RÈGLE MÉTIER : Bloquer uniquement si la réservation est déjà PAYÉE / VALIDÉE
        if (statutActuel.equals("PAYE") ||
                statutActuel.equals("PAYEE") ||
                statutActuel.equals("VALIDEE") ||
                statutActuel.equals("CONFIRMEE") ||
                statutActuel.equals("EMBARQUE")) {
            throw new RuntimeException("Annulation impossible : une réservation déjà payée ou confirmée ne peut pas être annulée.");
        }

        // Sécurité : éviter d'annuler une réservation déjà annulée
        if (statutActuel.equals("ANNULEE") || statutActuel.equals("ANNULE")) {
            throw new RuntimeException("Cette réservation est déjà annulée.");
        }

        // 3. Restituer les places disponibles pour le trajet
        if (r.getTrajet() != null && r.getTrajet().getPlacesDisponibles() != null) {
            int placesRendues = (r.getNombrePlaces() != null && r.getNombrePlaces() > 0) ? r.getNombrePlaces() : 1;
            r.getTrajet().setPlacesDisponibles(r.getTrajet().getPlacesDisponibles() + placesRendues);
            trajetRepository.save(r.getTrajet());
        ...
        // 4. Mettre à jour la demande de récupération VIP si elle existe
        demandeRecuperationRepository.findByReservationId(r.getId()).ifPresent(demande -> {
        ...
        notifierLeClient(r.getClient(), "❌ Votre réservation N° " + r.getId() + " a été annulée avec succès.");
    }
    public List<Reservation> listerToutes() { return reservationRepository.findAll(); }
    public List<Reservation> recupererParClient(Long clientId) { return reservationRepository.findByClientId(clientId); }
    public Reservation recupererParId(Long id) {
...
//
//    @Transactional
//    public void masquerReservationClient(Long id) {
//        Reservation r = reservationRepository.findById(id)
//                .orElseThrow(() -> new RuntimeException("Réservation introuvable avec l'ID : " + id));
//
//        String statutActuel = r.getStatut() != null ? r.getStatut().toUpperCase() : "";
//
//        // Sécurité : Interdire le masquage si la réservation est payée
//        if (statutActuel.equals("PAYE") ||
//                statutActuel.equals("PAYEE") ||
//                statutActuel.equals("VALIDEE") ||
//                statutActuel.equals("CONFIRMEE") ||
//                statutActuel.equals("EMBARQUE")) {
//            throw new RuntimeException("Une réservation payée ou confirmée ne peut pas être masquée de l'historique.");
//        }
//
//        // On masque la réservation pour l'affichage client
//        r.setMasquePourClient(true);
//        reservationRepository.save(r);
//    }

    @Transactional(readOnly = true)
    public List<Reservation> getHistoriqueClient(Long clientId) {
        // Utiliser la méthode filtrée qui exclut masquePourClient = true
        return reservationRepository.findHistoriqueClientActif(clientId);
    }

    /**
     * 🟢 1. Récupérer l'historique en ignorant les réservations masquées
     */
    @Transactional(readOnly = true)
    public List<HistoriqueVoyageDTO> obtenirHistoriqueClient(String email) {
        // Utiliser la méthode du repository avec le filtre WHERE masquePourClient = false
        List<Reservation> reservations = reservationRepository.findByClientEmailAndNotMasque(email);

        // Convertir vos entités en DTO
    ...
    /**
     * 🟢 2. Marquer la réservation comme masquée pour le client
     */
    @Transactional
    public void masquerReservationClient(Long id) {
        Reservation reservation = reservationRepository.findById(id)
        ...
        // Interdire le masquage des réservations validées/payées si votre règle métier l'exige
        if ("PAYE".equals(statut) || "CONFIRMEE".equals(statut) || "EMBARQUE".equals(statut)) {
            throw new RuntimeException("Impossible de retirer une réservation déjà payée ou confirmée de l'historique.");
        }
